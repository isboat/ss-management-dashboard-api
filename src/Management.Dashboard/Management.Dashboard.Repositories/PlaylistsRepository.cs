...
namespace Management.Dashboard.Repositories
{
    public class PlaylistsRepository : BaseRepository, IRepository<PlaylistModel>
    {
        ...
        public async Task<IEnumerable<PlaylistModel>> GetByFilterAsync(string tenantId, FilterDefinition<PlaylistModel> filter) =>
            await GetTenantCollection<PlaylistModel>(tenantId, CollectionName).Find(filter).ToListAsync();
    }
}