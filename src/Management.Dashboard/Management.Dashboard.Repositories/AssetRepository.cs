...
namespace Management.Dashboard.Repositories
{
    public class AssetRepository : BaseRepository, IRepository<AssetItemModel>
    {
        ...
        public async Task<IEnumerable<AssetItemModel>> GetByFilterAsync(string tenantId, FilterDefinition<AssetItemModel> filter) =>
            await GetTenantCollection<AssetItemModel>(tenantId, CollectionName).Find(filter).ToListAsync();
    }
}